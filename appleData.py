...
def compare_series(ref_serie, ts_series, output_dir, save_plots=False):
    ...
    for i in range(len(ts_norm)):
        ...
        dtw_matrix_restricted = compute_dtw_matrix_restricted(ref_norm, ts_norm[i], 40)
        ...
        if save_plots:
            ...
            plot_dtw_path(ref_norm, ts_norm[i], title=f'{i} DTW Matrix Standard', save_dir=output_dir, dtw_matrix=dtw_matrix)
            plot_dtw_path_restricted(ref_norm, ts_norm[i], 20, title=f'{i} DTW Matrix Restricted', save_dir=output_dir, dtw_matrix=dtw_matrix_restricted)
            ...
            plot_dtw_alignment(ref_norm, ts_norm[i], 'restricted', window=20, title=f'{i} DTW Alignment Restricted', save_dir=output_dir, dtw_matrix=dtw_matrix_restricted)
